...
min(cv_states$date)
max(cv_states$date)
# Date range: January 21st, 2020 to March 23rd 2023
# Range of cases: 1 to 12,169,158
# Ranfe of deaths: 0 to 104,277
```
...
-   Explore the pattern between $x$ and $y$ using `geom_smooth()`
    -   Explain what you see. Do you think `pop_density` is a correlate of `newdeathsper100k`?
        -   There seems to be a slight positive relationship between deaths per 100k and population density, meaning these variables are correlated.
...
-   Create a line chart of the `naive_CFR` for all states over time using `plot_ly()`
    -   Use the zoom and pan tools to inspect the `naive_CFR` for the states that had an increase in September. How have they changed over time?
        -   Some states have an increase in naive_CFR, deaths per new case, beginning in September, but is an overall decrease in this variable beginning in January 2022.
-   Create one more line chart, for Florida only, which shows `new_cases` and `new_deaths` together in one plot. Hint: use `add_layer()`
    -   Use hoverinfo to "eyeball" the approximate peak of deaths and peak of cases. What is the time delay between the peak of cases and the peak of deaths?
        -   The new case count peaked on January 10th, 2022 and the new death count peaked around September 20th, 2021.
...
For the new cases heat map, California, followed by Texas, New York, and Florida all stand out in January of 2022.

For the new cases per 100k heat map, all states have very similar rates, with a peak in January of 2022 and a small peak again around June of 2022.

Finally, looking at the new cases per 100k heat map over a two week span for the second half of 2021, Alaska stands out in October and Louisiana and Mississippi stand out in August,
...
-   Describe the difference in the pattern of the CFR.
    -   CFR appears to be the highest in Alaska followed by Montana and surrounding states in October of 2021 and other states have low CFR rates, while the CFR today is extremely low in all states. This makes sense as we have both vaccines and better Covid-19 containment and recovery methods now compared to 2021.
