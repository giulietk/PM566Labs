...
```{r}
str(met)
# Numeric: USAFID, WBAN, year, month, day, hour, min, lat, lon, elev, wind.dir, wind.sp, ceiling.ht, ceiling.ht.method, vis.dist, temp, dew.point, atm.press, atm.press.qc, rh, wind.sp.qc, vis.dist.qc, vis.var.qc, temp.qc, dew.point.qc
# Categorical: wind.dir.qc, wind.type.code, sky.cond, vis.var

# Variables of interest: year, month, day, hour, temp, elev, temp, wind.sp
```
...
```{r}
# The highest weather station has an elevation 4113 m
```
...
```{r}
summary(met$wind.sp)
# There are 79714 missing values for this variable
```
...
head(met) [,c(8:10, 24)]
# With a latitude of 38.767 and longitude of -104.3, El Paso County, Colorado has the coldest temperature readings
```

### **Does the range of values for elevation make sense? Why or why not?**

```{r}
# 4113 m is reasonable given Mount Whitney is 4417 m high and -13 m is also reasonable given the Badweather Basin is 86 m below sealevel.
```
...
```{r}
# Both temperature and wind speed slightly increases throughout the month
```
...
```{r}
# What are the variables that end in .qc? What do they mean?
```
...
boxplot(met$temp)
hist(table(met$temp))
plot(elev$hour,elev$dew.point)
# Dew point initially decreases then increases throughout the day, as expected
```
